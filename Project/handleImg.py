...
if __name__ == "__main__":
    img = Image.open('test.jpg')  #image 对象
    compress_image(img)
    ...
    img = Image.open('test.jpg')  #image 对象
    logo_watermark(img, 'logo.png')

    img = Image.open('test.jpg')  #image 对象
    text_watermark(img, 'Orangleliu')

    img = Image.open('test.jpg')  #image 对象
    resizeImg(img, dst_w=100, qua=85)
...
def resizeImg(img, dst_w=0, dst_h=0, qua=20):
    '''
    只给了宽或者高，或者两个都给了，然后取比例合适的
    如果图片比给要压缩的尺寸都要小，就不压缩了
    '''
    ...
    im.resize((newWidth,newHeight),Image.ANTIALIAS).save(img, quality=qua)
    print(os.path.basename(img) + u' 等比压缩完成')
    ...
    '''
import os
for par, dirs, files in os.walk('LoveAC爱冻鳗'):
	for file in files:
		if file.endswith('jpg') or file.endswith('png') or file.endswith('ico'):
			# psd BUG (can't open)
			imgfile = os.path.join(par, file)
			resizeImg(imgfile, getSize(imgfile)[0], getSize(imgfile)[1])
	# psd, png, jpg, ico